...
#======================================================================
model_df <-
  list.files(dir_root, "model_data_corn.rds", full.names = TRUE) %>%
  readRDS() %>% 
...
cmip6_tx_delta_change_data <-
  ...
  pivot_longer("370_su_M0":"370_sp_mea") %>%
...
cmip6_370 <-
  cmip6_tx_delta_change_data %>% 
  filter(model_scenario  == 370)

#predict function
predFun_ssp370 <- function(fit) {
  predict(fit,cmip6_370)
}
...
#store predictions
predictions_370 <- as.data.frame(t(bc370$t)) %>% 
  mutate(group_id = cmip6_370$group_id) 

#save file
saveRDS(predictions_370, file.path(dir_root,"prediction_corn_370.rds"))