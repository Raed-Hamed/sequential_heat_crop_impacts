...
cmip6_tx_delta_change_data <-
  ...
  pivot_longer("245_su_M0":"126_sp_mea") %>%
...
cmip6_126 <-
  cmip6_tx_delta_change_data %>% 
  filter(model_scenario  == 126)

#predict function
predFun_ssp126 <- function(fit) {
  predict(fit,cmip6_126)
}
...
#store predictions
predictions_126 <- as.data.frame(t(bc126$t)) %>% 
  mutate(group_id = cmip6_126$group_id) 

#save file
saveRDS(predictions_126, file.path(dir_root,"prediction_corn_126.rds"))