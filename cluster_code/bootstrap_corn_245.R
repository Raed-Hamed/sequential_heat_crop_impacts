...
cmip6_tx_delta_change_data <-
  ...
  pivot_longer("245_su_M0":"245_sp_mea") %>%
...
#predict function
predFun_ssp245 <- function(fit) {
  predict(fit,cmip6_245)
}
...
#store predictions
predictions_245 <- as.data.frame(t(bc245$t)) %>% 
  mutate(group_id = cmip6_245$group_id) 

#save file
saveRDS(predictions_245, file.path(dir_root,"prediction_corn_245.rds"))