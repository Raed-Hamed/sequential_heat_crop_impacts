...
#======================================================================
#load US spatial data
us_spatial_sf<-  function(x) {
  #x is either us_counties or us_states from usa boundaries package  
  x %>% 
    rename(County = name,
           State  = state_name) %>% 
...
#---------------------------------------------------------------------
# # calc yield prediction  based on the model coefficients
...
#---------------------------------------------------------------------
#Modified/hacked version of the lme4:::coef.merMod() function
...
#     
#     facet_wrap( ~ term) +
#     scale_fill_continuous_diverging("Blue-Red 3", limit = c(-1.5, 1.5)) +
